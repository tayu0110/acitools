...
use acitools::{
    Client, FvBD, FvBDEndpoint, FvTenant, FvTenantEndpoint, L2UnknownUnicast, L3UnknownMulticast,
...
async fn bd_test() {
    ...
    let res = FvBD::new("test-bd", "test-tenant")
        .create(FvBDEndpoint::MoUni, &client)
        .await;
    assert!(res.is_ok());

    let bds = FvBD::get(FvBDEndpoint::ClassAll).send(&client).await;
    assert!(bds.is_ok());
    assert!(bds
        .unwrap()
        .into_iter()
        .any(|bd| bd.attributes().name() == "test-bd"));
    ...
    assert!(bds
        .unwrap()
        .into_iter()
        .all(|bd| bd.attributes().name() != "test-bd"));
